-- ============================================================
-- 학원용 부정 시청 방지 학습 시스템 - Supabase 스키마 (재생성용)
...
-- [2] 기존 테이블 삭제 (FK 순서 고려: assignments → videos, profiles)
drop table if exists public.assignments cascade;
drop table if exists public.videos cascade;
drop table if exists public.profiles cascade;

-- [3] profiles (auth.users와 1:1, role로 권한 구분, full_name으로 학생 로그인)
...
-- [6] 관리자 페이지에서 학생명 표시용: assignments → profiles 조인
...
-- [9] 현재 사용자 role 조회 함수 (RLS 무한 재귀 방지용, profiles 읽을 때 RLS 안 탐)
...
-- [14] 관리자 지정 (실행 후 아래에서 이메일만 수정해서 한 번 실행)
